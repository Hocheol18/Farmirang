...
import { fetchDonationDataWithCompony, fetchDonorData } from "@/api/farm-donation";
...
export default function Page() {
    ...
    const [fetchWholeList, setFetchWholeList] = useState<fetchDonationDataType[]>()

    useEffect(() => {
        const fetchDonorPerson = async () => {
            const res = await fetchDonationDataWithCompony({ cursor: 0, size: 30, user: Number(memberId) })
            const fetchData = await Promise.all(res.data.posts.map((item) => fetchDonorData(item.id)));
            const filteredPosts = res.data.posts.filter((post, index) => fetchData[index].data.donors.length >= 1);
            const filteredData = fetchData.filter((donorData) => donorData.data.donors.length >= 1);
                        ...
                        <div className="inline-flex items-start justify-center gap-[40px] relative flex-[0_0_auto]">
                            ...
                            <div className="inline-flex flex-col items-start justify-center gap-[15px] p-[15px] relative flex-[0_0_auto] border !rounded-[10px] border-solid border-graygray-300  w-[920px]">
                                ...
                                <div className="w-full flex flex-col gap-[20px] justify-center">
                                    {fetchWholeList && fetchDonationList ? fetchWholeList.map((item, idx: number) => (
